...
Open **http://localhost:5174**. Read the invitation from ignored `.local/invite.txt` and share it only with the intended reviewer. An Anthropic API key is separate from a Claude web subscription and requires API billing access. Never commit keys. `docker compose up --build` provides an independent local database/API alternative; live mode defaults off. Without a backend, the public shell still works, but its empty replay catalogue is not a completed live demo.
...
**Development in progress. Not production ready.** Python/FastAPI performs deterministic artwork inspection, Claude orchestrates bounded visual analysis, and React/TypeScript presents versioned findings and human-approved proofs.
